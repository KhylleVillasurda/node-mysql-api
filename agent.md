...
*   **Start (Production):** `pnpm start` (runs `node dist/server.js`)
*   **Start (Development):** `pnpm dev` (runs `nodemon --exec ts-node src/server.ts`)
*   **Configuration:** Update `.env` for database, JWT, and SMTP settings.
